...
```bash
npx --yes github:studio121-develop/controllo-ai <dominio> --json
```

Riporta all'utente il punteggio e il giudizio, poi solo i controlli non verdi con `dettaglio` e `spiegazione` così come sono (sono già scritti per chi non è tecnico). Non inventare controlli che il pacchetto non fa. Se il sito non risponde, dillo.
