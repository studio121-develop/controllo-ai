...
export const ROBOT_CHE_RISPONDONO = ["OAI-SearchBot", "ChatGPT-User", "PerplexityBot", "Perplexity-User", "Claude-SearchBot", "Claude-User", "Meta-ExternalAgent", "Amazonbot", "DuckAssistBot", "YouBot", "MistralAI-User"] as const;
/** Chi si ADDESTRA sui testi: non porta nessuno. Bloccarlo è una scelta. */
export const ROBOT_CHE_SI_ADDESTRANO = ["GPTBot", "ClaudeBot", "CCBot", "Bytespider", "Applebot-Extended", "omgili", "Diffbot", "cohere-ai", "anthropic-ai", "Meta-ExternalFetcher"] as const;
/** Google-Extended: governa Gemini e le risposte AI di Google con fonti. A metà strada: bloccarlo costa visibilità su Gemini. */
export const ROBOT_GOOGLE_AI = ["Google-Extended"] as const;
/** I motori classici, che oggi alimentano anche le risposte AI (AI Overview, Copilot). Bloccarli è grave. */
export const MOTORI = ["Googlebot", "Bingbot"] as const;

type Regola = Readonly<{ tipo: "allow" | "disallow"; percorso: string }>;
type Gruppo = Readonly<{ agenti: readonly string[]; regole: readonly Regola[] }>;

export type Robots = Readonly<{ gruppi: readonly Gruppo[]; sitemap: readonly string[]; righeNonCapite: number }>;
...
export function analizzaRobots(testo: string): Robots {
  ...
  const chiudi = () => { if (agenti.length) gruppi.push({ agenti, regole }); agenti = []; regole = []; inRegole = false; };
  for (const grezza of testo.split(/\r?\n/)) {
    const riga = grezza.replace(/#.*$/, "").trim();
    if (!riga) continue;
    const m = /^([a-z-]+)\s*:\s*(.*)$/i.exec(riga);
    if (!m) { righeNonCapite++; continue; }
    ...
    if (campo === "user-agent") {
      ...
      inRegole = true;
      if (agenti.length) regole.push({ tipo: campo, percorso: valore });
...
function gruppoPer(robots: Robots, agente: string): Gruppo | null {
  ...
  if (prefisso) return prefisso;
  return robots.gruppi.find((g) => g.agenti.includes("*")) ?? null;
}

function corrisponde(percorso: string, regola: string): boolean {
...
export function permesso(robots: Robots, agente: string, percorso = "/"): boolean {
  const g = gruppoPer(robots, agente);
  if (!g) return true;
  let migliore: Regola | null = null;
  for (const r of g.regole) {
    if (!corrisponde(percorso, r.percorso)) continue;
    if (!migliore || r.percorso.length > migliore.percorso.length || (r.percorso.length === migliore.percorso.length && r.tipo === "allow")) migliore = r;
  }
  return !migliore || migliore.tipo === "allow";
}

/** Fra i robot indicati, quelli a cui la home è vietata. */
export function robotBloccati(robots: Robots, elenco: readonly string[], percorso = "/"): readonly string[] {
