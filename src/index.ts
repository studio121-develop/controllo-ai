...
export type Controllo = Readonly<{ chiave: string; nome: string; esito: Esito; dettaglio: string; spiegazione: string }>;
export type Risultato = Readonly<{ url: string; punteggio: number; giudizio: string; controlli: readonly Controllo[]; controllatoIl: string }>;
...
export const ROBOT_CHE_RISPONDONO = ["OAI-SearchBot", "PerplexityBot", "Claude-SearchBot", "Claude-User", "ChatGPT-User"] as const;
export const ROBOT_CHE_SI_ADDESTRANO = ["GPTBot", "ClaudeBot", "Google-Extended", "CCBot", "Bytespider", "Applebot-Extended"] as const;
...
function controllo(chiave: string, nome: string, esito: Esito, dettaglio: string): Controllo {
...
export function robotBloccati(robots: string, robotDaControllare: readonly string[]): readonly string[] {
...
export function tipiJsonLd(html: string): readonly string[] {
  const tipi: string[] = [];
  const raccogli = (nodo: unknown): void => {
    if (Array.isArray(nodo)) return nodo.forEach(raccogli);
    if (!nodo || typeof nodo !== "object") return;
    const o = nodo as Record<string, unknown>;
    const t = o["@type"];
    if (typeof t === "string") tipi.push(t);
    if (Array.isArray(t)) t.forEach((x) => typeof x === "string" && tipi.push(x));
    if (o["@graph"]) raccogli(o["@graph"]);
  };
  for (const m of html.matchAll(/<script[^>]*type=["']application\/ld\+json["'][^>]*>([\s\S]*?)<\/script>/gi)) {
    try { raccogli(JSON.parse(m[1] ?? "")); } catch { /* blocco malformato: si ignora */ }
  }
  return tipi;
}

export function analizzaHome(html: string): readonly Controllo[] {
...
async function testo(chiama: Chiamata, url: string): Promise<{ stato: number; corpo: string }> {
  try {
...
export function giudizio(punteggio: number): string {
...
export async function controlla(sito: string, chiama: Chiamata = (u, i) => fetch(u, i)): Promise<Risultato> {
