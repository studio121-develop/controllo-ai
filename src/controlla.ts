...
export async function controlla(sito: string, opzioni: Opzioni = {}): Promise<Risultato> {
  ...
  if (homeOk) {
    ...
    controlli.push(controllo("chisiamo", "Si capisce chi c'è dietro il sito", esito,
      esito === "bene" ? `sì: ${vie.join(", ")}` : esito === "neutro" ? "c'è una pagina contatti, ma nessuna pagina o scheda che dica chi siete: consigliata" : "dalla pagina principale non si arriva a chi siete né a come contattarvi"));
  ...
  if (homeOk && modalita === "completo") {
    ...
    const nomePagina = (u: string) => { try { const p = new URL(u).pathname; return p === "/" ? "pagina principale" : p; } catch { return u; } };
    controlli.push(controllo("faq", "Ci sono domande e risposte", conMarkup ? "bene" : "neutro",
      conMarkup ? `presenti in ${nomePagina(conMarkup.url)}, con il markup che le AI riconoscono` : senzaMarkup ? `${senzaMarkup.titoliDomanda} domande in ${nomePagina(senzaMarkup.url)}, ma senza il markup FAQPage: aggiungerlo` : "non trovate nelle pagine lette: facoltative"));
