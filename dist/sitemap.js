...
export function urlDaSitemap(xml, limite = 50) {
    return [...xml.matchAll(/<loc>\s*([^<\s]+)\s*<\/loc>/gi)].map((m) => m[1]).slice(0, limite);
}
export function sitemapIndice(xml) {
...
export function linkInterni(html, base, limite = 40) {
    ...
    for (const m of html.matchAll(/<a[^>]+href=["']([^"'#]+)["']/gi)) {
        ...
        if (u.hostname.replace(/^www\./, "") !== origine.hostname.replace(/^www\./, ""))
            continue;
        if (!/^https?:$/.test(u.protocol) || NON_PAGINE.test(u.pathname + u.search))
            continue;
        u.hash = "";
        u.search = "";
        const chiave = u.toString().replace(/\/$/, "");
        if (chiave === origine.toString().replace(/\/$/, "") || visti.has(chiave))
            continue;
        visti.add(chiave);
        esito.push(u.toString());
        if (esito.length >= limite)
            break;
    }
    return esito;
}
/** Le pagine «chi siamo» e «contatti», cercate per nome nei link o nel testo dei link. */
export function paginePresentazione(html, base) {
    const trova = (re) => {
        for (const m of html.matchAll(/<a[^>]+href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi)) {
            const href = m[1];
            const testo = m[2].replace(/<[^>]+>/g, " ");
            if (re.test(href) || re.test(testo)) {
                try {
...
export function scegliPagine(daSitemap, daLink, home, quante = 5) {
    ...
    for (const u of candidate)
        if (!perChiave.has(norm(u)))
            perChiave.set(norm(u), u); // la sitemap vince sui link
