...
export function linguaDelTesto(testo) {
    const conta = new Map();
    for (const p of testo.toLowerCase().split(/\s+/))
        conta.set(p, (conta.get(p) ?? 0) + 1);
    const punteggi = Object.entries(SPIE).map(([lingua, spie]) => [lingua, spie.reduce((a, s) => a + (conta.get(s) ?? 0), 0)]).sort((a, b) => b[1] - a[1]);
    const [prima, seconda] = punteggi;
    return prima && prima[1] >= 5 && (!seconda || prima[1] >= seconda[1] * 1.5) ? prima[0] : null;
}
export function leggiMeta(html) {
...
export function controllaPagina(html, url, xRobots, home) {
    ...
    c.push(controllo("titolo", "La pagina ha un titolo chiaro", !t ? "male" : titoloGenerico || titoloSoloNome ? "male" : t.length < 15 || t.length > 70 ? "neutro" : "bene", !t ? "manca il titolo" : titoloGenerico ? `«${t}»: un titolo generico, non dice né chi sei né cosa fai` : titoloSoloNome ? `«${t}»: dice solo il nome, non cosa fai` : `«${t}»${t.length > 70 ? " (lungo: Google lo taglia)" : t.length < 15 ? " (corto)" : ""}`, dove));
    const d = m.descrizione;
    const descDebole = !!d && (DESCRIZIONI_VUOTE.test(d) || d.toLowerCase() === t.toLowerCase());
    c.push(controllo("descrizione", "C'è una descrizione di cosa fai", !d ? "male" : descDebole ? "male" : d.length < 50 || d.length > 170 ? "neutro" : "bene", !d ? "manca la descrizione" : descDebole ? `«${d.slice(0, 60)}…»: generica o uguale al titolo` : `${d.length} caratteri${d.length < 50 ? ": troppo corta" : d.length > 170 ? ": lunga, viene tagliata" : ""}`, dove));
    const h1Vuoti = m.h1.filter((x) => !x).length;
    c.push(controllo("h1", "Un titolo principale nella pagina", m.h1.length === 1 && !h1Vuoti ? "bene" : m.h1.length === 0 ? "male" : "neutro", m.h1.length === 1 && !h1Vuoti ? `«${m.h1[0].slice(0, 70)}»` : m.h1.length === 0 ? "nessuno" : h1Vuoti ? "c'è, ma è un'immagine senza testo" : `${m.h1.length} titoli principali: meglio uno`, dove));
    if (home) {
        c.push(controllo("lingua", "La lingua del sito è dichiarata", m.lang ? (m.linguaTesto && !m.lang.startsWith(m.linguaTesto) ? "neutro" : "bene") : "male", m.lang ? (m.linguaTesto && !m.lang.startsWith(m.linguaTesto) ? `dichiara «${m.lang}» ma il testo sembra in «${m.linguaTesto}»` : m.lang) : "non dichiarata"));
        c.push(controllo("testo", "C'è abbastanza testo da leggere", m.jsLento ? "male" : m.parole < 150 ? "neutro" : "bene", m.jsLento ? "la pagina si costruisce solo con programmi nel browser: alle AI arriva quasi vuota" : `${m.parole} parole${m.parole < 150 ? ": poche, le AI hanno poco da capire" : ""}`));
        const cd = m.chiEDove;
        const trovati = [cd.telefono && "telefono", cd.email && "email", cd.indirizzo && "indirizzo", cd.cap && "città"].filter(Boolean);
        c.push(controllo("contatti", "Dice chi sei e dove sei", trovati.length >= 2 ? "bene" : trovati.length === 1 ? "neutro" : "male", trovati.length ? `trovati: ${trovati.join(", ")}` : "nessun telefono, indirizzo o email nella pagina"));
        const canon = m.canonical ? (() => { try {
            return new URL(m.canonical, url);
        }
        catch {
            return null;
        } })() : null;
        const altroDominio = canon && canon.hostname.replace(/^www\./, "") !== dominio;
        c.push(controllo("canonical", "La pagina indica il suo indirizzo ufficiale", !m.canonical ? "neutro" : altroDominio ? "male" : "bene", !m.canonical ? "non indicato: va bene, ma con www e senza www conviene dirlo" : altroDominio ? `punta a un altro sito (${canon.hostname}): Google e le AI attribuiscono la pagina a quello` : "indicato"));
        ...
        if (m.refresh)
            c.push(controllo("refresh", "La pagina rimanda altrove da sola", "male", "c'è un rinvio automatico: le AI si fermano qui"));
    }
    return c;
}
