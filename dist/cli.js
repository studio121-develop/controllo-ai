...
export async function main(argv) {
    ...
    for (const c of r.controlli) {
        ...
        if (c.esito !== "bene")
            console.log(`   ${c.spiegazione}${argv.includes("--soluzioni") ? `\n   → ${c.comeRisolvere}` : ""}`);
    }
    ...
    if (altre.length) {
        ...
        for (const p of altre)
            console.log(`  ${p.url}${p.stato !== 200 ? ` (HTTP ${p.stato})` : ""}: ${p.controlli.filter((c) => c.esito !== "bene").map((c) => `${LUCE[c.esito]} ${c.nome.toLowerCase()}`).join(", ") || "non leggibile"}`);
    }
    return 0;
}
